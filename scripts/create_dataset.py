...
def main(args):
    ...
    if args.dataset_path:
        if args.dataset_name:
            hdf5_name = args.dataset_name + "_" + os.path.basename(bddl_file_name).replace(".bddl", "_demo.hdf5")
        else:
            hdf5_name = os.path.basename(bddl_file_name).replace(".bddl", "_demo.hdf5")
        hdf5_path = os.path.join(args.dataset_path, hdf5_name)
        if os.path.exists(hdf5_path) and not args.overwrite:
            print(f"The dataset at {hdf5_path} already exists. Skipping...")
            return
    else:
    ...
    for (i, ep) in enumerate(demos):
        ...
        if 'rand' not in args.dataset_path:
            rewards[-1] = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--demo-file",
        default=os.path.join(os.getenv("HOME"), "code/sheeprl/demonstration_data2"),
    )
    ...
    parser.add_argument(
        "--dataset-path",
        type=str,
        default=os.path.join(os.getenv("HOME"), "data/libero/datasets/rand_libero_90"),
    )

    parser.add_argument(
        "--dataset-name",
        type=str,
        default=None #"rand_start", #""
    )
    ...
    if os.path.isfile(args.demo_file):
        main(args)
    elif os.path.isdir(args.demo_file):
        hdf5_files = glob(os.path.join(args.demo_file, '**/*.hdf5'), recursive=True)
        for hdf5_file in hdf5_files:
            args.demo_file = hdf5_file
            main(args)
    else:
        raise ValueError(f"{args.demo_file} is neither a file nor a directory")
    # main(args)
